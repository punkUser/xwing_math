...
public struct TokenState2
{
    ...
    int opCmp(ref const TokenState2 s) const
    {
        return memcmp(&this, &s, TokenState2.sizeof);
    }
}
...
public alias SimulationStateMap2 = double[SimulationState2];

public alias ForkDiceDelegate2 = SimulationState2 delegate(SimulationState2 state);

// Utility to either insert a new state into the map, or accumualte probability if already present
public void append_state(ref SimulationStateMap2 map, SimulationState2 state, double probability)
{
    auto i = (state in map);
    if (i !is null)
    {
...
public SimulationStateMap2 roll_attack_dice(bool initial_roll)(
    ref const(SimulationStateMap2) prev_states,
    ForkDiceDelegate2 cb,
    ubyte initial_roll_dice = 0)
{
    SimulationStateMap2 next_states;
        ...
        for (int crit = 0; crit <= count; ++crit)
        {
            for (int hit = 0; hit <= (count - crit); ++hit)
            {
                for (int focus = 0; focus <= (count - crit - hit); ++focus)
                {
                    ...
                    SimulationState2 new_state = state;
                    if (initial_roll)
                    {
...
public SimulationStateMap2 roll_defense_dice(bool initial_roll)(
    ref const(SimulationStateMap2) prev_states,
    ForkDiceDelegate2 cb, 
    ubyte initial_roll_dice = 0)
{
